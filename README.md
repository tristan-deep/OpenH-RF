...
3. **Contribute Data** — Once approved, prepare your dataset in the OpenH-RF format ([`zea` data-acquisition spec](https://zea--358.org.readthedocs.build/en/358/data-acquisition.html)) along with a datacard specifying the CC BY 4.0 license. Approved contributors are given a dedicated shared storage location (S3 or Google Drive) for delivery and a Discord channel for coordination.
